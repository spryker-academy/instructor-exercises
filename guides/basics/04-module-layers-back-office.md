...
#### 1.1 Add a MessageCriteriaTransfer Definition
...
Open `src/SprykerAcademy/Shared/HelloWorld/Transfer/helloworld.transfer.xml` and add a DTO named **MessageCriteria** with:
...
Open `src/SprykerAcademy/Zed/HelloWorld/Persistence/HelloWorldPersistenceFactory.php`. In the `createMessageQuery()` method, return an instance of `PyzMessageQuery`.
...
Open `src/SprykerAcademy/Zed/HelloWorld/Persistence/HelloWorldRepository.php`. In the `findMessage()` method:
1. Get the message query from the factory
2. If the criteria has an `idMessage`, find by ID and return the result
...
5. Map the entity to a DTO using the MessageMapper and return it
...
Open `src/SprykerAcademy/Zed/HelloWorld/Persistence/HelloWorldEntityManager.php`. In the `createMessage()` method:
1. Map the DTO to an entity using the MessageMapper
2. Call `$messageEntity->save()`
...
#### 2.1 Add MessageResponseTransfer Definition

Wrap the DTO in a `*ResponseTransfer` to include success/error information.

**Coding time:**

Open `src/SprykerAcademy/Shared/HelloWorld/Transfer/helloworld.transfer.xml` and add a DTO named **MessageResponse** with:
...
Open `src/SprykerAcademy/Zed/HelloWorld/Business/Writer/MessageWriter.php`:
- Inject `HelloWorldEntityManagerInterface` through the constructor
...
Open `src/SprykerAcademy/Zed/HelloWorld/Business/Reader/MessageReader.php`:
- Inject `HelloWorldRepositoryInterface` through the constructor
- In the `findMessage()` method, use the Repository to find a message by criteria
- Create a `MessageResponseTransfer` and set `message` and `isSuccessful` based on the result
- Return the response
...
Open `src/SprykerAcademy/Zed/HelloWorld/Business/HelloWorldBusinessFactory.php`. Instantiate `MessageWriter` and `MessageReader` with their dependencies.
...
Open `src/SprykerAcademy/Zed/HelloWorld/Business/HelloWorldFacade.php`. Implement `createMessage()` and `findMessage()` using the factory's writer and reader.
...
Open `src/SprykerAcademy/Zed/HelloWorld/Communication/Controller/MessageController.php`. Complete `addAction()`:
1. Instantiate `MessageCriteriaTransfer` and set the message name from the query parameter
2. Use the Facade to find the message
3. If no message is found, create a new `MessageTransfer` with the name and use the Facade to create it

#### 3.2 Template for the Back Office

The controller passes `message` (a `MessageTransfer`) to the template. Access object properties with dot notation: `objectName.objectProperty`.

**Coding time:**

Open `src/SprykerAcademy/Zed/HelloWorld/Presentation/Message/add.twig`. Replace the placeholder `'show-message-name-here'` with the actual message name.
...
Visit: http://backoffice.eu.spryker.local/hello-world/message/add
...
```bash
docker/sdk cli vendor/bin/codecept run -c tests/SprykerAcademyTest/Zed/HelloWorld/ Exercise4
```

This tests the MessageReader and MessageWriter business logic. All tests should pass if your implementation is correct.
